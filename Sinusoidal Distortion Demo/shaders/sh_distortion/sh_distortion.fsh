...
void main()
{
	...
	//Tile the graphic
	float img_x = pixel_width * image_width - pixel_width;
	float img_y = pixel_height * image_height - pixel_height;
	texcoord.x = texcoord.x - img_x * floor(texcoord.x / img_x);
	texcoord.y = texcoord.y - img_y * floor(texcoord.y / img_y);
